...
class BertData():
    ...
    def preprocess(self, src, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        return src_subtoken_idxs, sent_labels, segments_ids, cls_ids, src_txt
...
def _format_to_bert(params):
    ...
    datasets = []
    for d in jobs:
        source, labels = d['src'], d['label']
        sent_labels = [float(i) for i in labels]
    
        #sent_labels = greedy_selection(source[:args.max_src_nsents], tgt, 3)
        if (args.lower):
            source = [' '.join(s).lower().split() for s in source]
    #    tgt = [' '.join(s).lower().split() for s in tgt]
        b_data = bert.preprocess(source, sent_labels, use_bert_basic_tokenizer=args.use_bert_basic_tokenizer,
                             is_test=is_test)
    # b_data = bert.preprocess(source, tgt, sent_labels, use_bert_basic_tokenizer=args.use_bert_basic_tokenizer)

        if (b_data is None):
            continue
        src_subtoken_idxs, sent_labels, segments_ids, cls_ids, src_txt = b_data
